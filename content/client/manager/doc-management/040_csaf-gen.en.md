...
- The heart of a CSAF document is the connection between products and vulnerabilities. This is communicated via the "products" array. Each entry has the following fields:
  ...
  - The "cpe" and "purl" fields can be used to specify a [Common Platform Enumeration](https://de.wikipedia.org/wiki/Common_Platform_Enumeration) and [Package Uniform Resource Locator](https://github.com/package-url/purl-spec) value for the product. If either is provided, BOMnipotent creates a [product identification helper](https://docs.oasis-open.org/csaf/csaf/v2.0/os/csaf-v2.0-os.html#3133-full-product-name-type---product-identification-helper) entry for the product.

  BOMnipotent uses this data to generate a "product_tree" of products mentioned in the input. This tree contains a layer for "vendor", for "product_name", and for "product_version". If "add_vendor_branch" has been set to false, the "vendor" layer is omitted, since this information is already found elsewhere in the CSAF.
  
  Furthermore the "vulnerabilities" entry of the CSAF is created, where each product is listed with its status.
